...
layout(location = 1) out vec4 rtxRayOrigin;
layout(location = 2) out vec4 rtxRayDir;
...
void main() {
    ...
    if (depth == 1.0) {
        ...
        blockLight += getHandheldLighting(rayOrigin, rightHand, leftHand) * blocklightColor;
        ...
        if (max(specularColor.r, max(specularColor.g, specularColor.b)) > 0.01) {            
            ...
            vec3 reflection = ssr(viewPos + viewNormal * 0.02, viewRayDir, skyDir, sunDir, moonDir, timeMod);
            ...
            rtxRayOrigin = vec4(rayOrigin - cameraPosition, 1);
            rtxRayDir = vec4(rayDir, 1);
    ...
    if (gl_FragCoord.y >= 40 && gl_FragCoord.y < 60)
        color.rgb = unpackUnorm3x10(codes[int(texcoord.x * float(blocks.triCount))].mortonCodesA);
